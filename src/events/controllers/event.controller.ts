...
export class EventController {
    public static getEvents = async (request: Request, response: Response) => {
        try {
            const [events, total] = await EventService.getEvents();
            return response.send({ events, total })
        } catch (e) {
            console.log(e)
    ...
    public static insertEvent = async (request: Request, response: Response) => {
        try {
            const event = await EventService.insertEvent(request);
            return response.status(200).send({ message: 'Success', event });
        } catch (e) {
            console.log(e)
            return response.send({ status: 500, message: `Server error`});
        }
    ...
    //
    // public static DeleteEvent = async (request: Request, response: Response) => {
    //     try {
    //         await EventService.deleteEvent(request,response);
    //         return response.send({ status: 204, message: `Event deleted successfully`})
    //     } catch (e) {
    //         console.log(e)
    //         return response.send({ status: 500, message: `Server error`});
    //     }
